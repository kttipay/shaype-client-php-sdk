...
class KYCAPIApi
{
    ...
    ): array
    {
        ...
        try {
            ...
            switch($statusCode) {
                case 200:
                    if (in_array('\Shaype\Client\Model\ConfirmationResponse', ['\SplFileObject', '\Psr\Http\Message\StreamInterface'])) {
     ...
     *
     * @param  \Shaype\Client\Model\UserConsentRequestBody|null $user_consent_request_body user_consent_request_body (optional)
    ...
    public function createCase(
        ?\Shaype\Client\Model\UserConsentRequestBody $user_consent_request_body = null,
        string $contentType = self::contentTypes['createCase'][0]
    ): \Shaype\Client\Model\CreateCaseExternalResponse
    {
        list($response) = $this->createCaseWithHttpInfo($user_consent_request_body, $contentType);
        return $response;
    }
     ...
     * Create new identity verification case and first submission
     *
     * @param  \Shaype\Client\Model\UserConsentRequestBody|null $user_consent_request_body (optional)
     * @param  string $contentType The value for the Content-Type header. Check self::contentTypes['createCase'] to see the possible values for this operation
     *
     * @throws ApiException on non-2xx response or if the response body is not in the expected format
     * @throws InvalidArgumentException
    ...
    {
        $request = $this->createCaseRequest($user_consent_request_body, $contentType);
        ...
        try {
            ...
            switch($statusCode) {
                case 200:
                    if (in_array('\Shaype\Client\Model\CreateCaseExternalResponse', ['\SplFileObject', '\Psr\Http\Message\StreamInterface'])) {
                        $content = $response->getBody(); //stream goes to serializer
                    } else {
                        $content = (string) $response->getBody();
    ...
    {
        return $this->createCaseAsyncWithHttpInfo($user_consent_request_body, $contentType)
            ->then(
                function ($response) {
                    return $response[0];
                }
    ...
    public function createCaseAsyncWithHttpInfo(
        $user_consent_request_body = null,
        string $contentType = self::contentTypes['createCase'][0]
    ): PromiseInterface
    {
        $returnType = '\Shaype\Client\Model\CreateCaseExternalResponse';
    ...
    ): Request
    {
        ...
        // for model (json/xml)
        if (isset($user_consent_request_body)) {
            if (stripos($headers['Content-Type'], 'application/json') !== false) {
                # if Content-Type contains "application/json", json_encode the body
                $httpBody = \GuzzleHttp\Utils::jsonEncode(ObjectSerializer::sanitizeForSerialization($user_consent_request_body));
            } else {
                $httpBody = $user_consent_request_body;
            }
        } elseif (count($formParams) > 0) {
            if ($multipart) {
                $multipartContents = [];
